...
// Direct mode with all three args: --clnx, --bitfile, --flash-version
TEST_F(ParseArgumentsTest, DirectMode_WithClnx)
{
    Args a { "hsb_flasher", "-H", "192.168.0.2",
        "--clnx", "clnx.bin", "--bitfile", "cpnx.bin",
        "--flash-version", "2504" };
    ASSERT_TRUE(parse_arguments(a.argc(), a.argv(), ctx_));
    EXPECT_TRUE(ctx_.direct_flash.enabled());
...
// Standard mode with IP but no -t/--target-version — valid; version is auto-detected at runtime
TEST_F(ParseArgumentsTest, MissingTargetVersion_AutoDetect_Succeeds)
{
    Args a { "hsb_flasher", "-H", "192.168.0.2" };
    ASSERT_TRUE(parse_arguments(a.argc(), a.argv(), ctx_));
    EXPECT_TRUE(ctx_.target_version.empty());
    EXPECT_FALSE(ctx_.reapply);
}
...
// --bitfile provided without --flash-version (valid: uses device's discovered version)
TEST_F(ParseArgumentsTest, DirectMode_CpnxOnly)
{
    Args a { "hsb_flasher", "-H", "192.168.0.2", "--bitfile", "cpnx.bin" };
    ASSERT_TRUE(parse_arguments(a.argc(), a.argv(), ctx_));
    EXPECT_TRUE(ctx_.direct_flash.enabled());
    EXPECT_EQ(ctx_.direct_flash.cpnx_path, "cpnx.bin");
...
// --flash-version provided without --bitfile
...
{
    Args a { "hsb_flasher", "-H", "192.168.0.2",
        "--bitfile", "cpnx.bin", "--flash-version", "zzzz" };
    EXPECT_FALSE(parse_arguments(a.argc(), a.argv(), ctx_));
}
...
// --reapply is parsed and sets the reapply field
TEST_F(ParseArgumentsTest, Reapply_Flag_Parsed)
{
    Args a { "hsb_flasher", "-H", "192.168.0.2", "-t", "2507", "--reapply" };
    ASSERT_TRUE(parse_arguments(a.argc(), a.argv(), ctx_));
    EXPECT_TRUE(ctx_.reapply);
}

// --reapply is valid in combination with direct flash mode
TEST_F(ParseArgumentsTest, Reapply_WithDirectMode_Parsed)
{
    Args a { "hsb_flasher", "-H", "192.168.0.2", "--bitfile", "cpnx.bin", "--reapply" };
    ASSERT_TRUE(parse_arguments(a.argc(), a.argv(), ctx_));
    EXPECT_TRUE(ctx_.reapply);
    EXPECT_TRUE(ctx_.direct_flash.enabled());
}

// --reapply is valid without -t (auto-detect + reapply)
TEST_F(ParseArgumentsTest, Reapply_WithoutTargetVersion_Parsed)
{
    Args a { "hsb_flasher", "-H", "192.168.0.2", "--reapply" };
    ASSERT_TRUE(parse_arguments(a.argc(), a.argv(), ctx_));
    EXPECT_TRUE(ctx_.reapply);
    EXPECT_TRUE(ctx_.target_version.empty());
}
