...
// ── --bitfile only (no --clnx, no --flash-version) ─────────────────────
...
// --reapply: flasher is found even when device version matches target
TEST(Reapply, SameVersion_WithReapply_FindsFlasher)
{
    auto ctx = make_context("192.168.0.2", "2507");
    ctx.reapply = true;
    ...
    // Even though versions match, reapply means we should still get a flasher
    auto flasher = get_flasher(ctx);
    EXPECT_NE(flasher, nullptr);
}

// --reapply: version check is the caller's responsibility;
// get_flasher itself always returns a flasher if UUID+version match
TEST(Reapply, SameVersion_WithoutReapply_FlasherStillAvailable)
{
    auto ctx = make_context("192.168.0.2", "2507");
    ...
    // get_flasher doesn't enforce version-match skipping — main.cpp does.
    // Confirm flasher is still available regardless.
    auto flasher = get_flasher(ctx);
    EXPECT_NE(flasher, nullptr);
}
