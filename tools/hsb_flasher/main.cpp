...
int main(int argc, char* argv[])
{
    ...
    // Manifest lookup must happen before version check so we can auto-detect
    // the latest version when -t is omitted.
    ...
    // Auto-select latest version when -t was not specified
    if (context.target_version.empty()) {
        if (!find_latest_version(context)) {
            log_info("Error: Failed to determine latest version from manifest");
            return EXIT_FAILURE;
        }
        log_info("Auto-selected latest version: 0x" + context.target_version);
    }

    int64_t current_version = context.enumeration_metadata.get<int64_t>("hsb_ip_version").value_or(0);
    if (!context.reapply && current_version == std::stoi(context.target_version, nullptr, 16)) {
