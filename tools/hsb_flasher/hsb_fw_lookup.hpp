...
/**
 * @brief Determine the latest available firmware version from the manifest.
 *
 * Scans all entries in @c firmware_versions within the YAML file at
 * @p context.firmware_info_path and selects the highest version number.
 * On success, writes the version as a lowercase hex string (without 0x prefix)
 * into @p context.target_version.
 *
 * Must be called after a successful find_manifest_by_uuid().
 *
 * @param[in,out] context  Manager context with a valid @c firmware_info_path.
 * @return true if a version was found and set, false otherwise.
 */
bool find_latest_version(hsb_flasher_context& context);
