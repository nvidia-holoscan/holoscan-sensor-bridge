...
void print_usage(const char* program_name)
{
    std::cout
        ...
        << "       " << program_name << " -H HOLOLINK_IP --bitfile PATH [--clnx PATH] [--flash-version VER] [OPTIONS]" << std::endl
        << std::endl
        << "Examples:" << std::endl
        << "  " << program_name << " -H 192.168.0.2                     (flash to latest available version)" << std::endl
        << "  " << program_name << " -H 192.168.0.2 -t 2507" << std::endl
        << "  " << program_name << " -H 192.168.0.2 --bitfile cpnx.bin" << std::endl
        << "  " << program_name << " -H 192.168.0.2 --clnx clnx.bin --bitfile cpnx.bin --flash-version 2507" << std::endl
        << std::endl
        << "Required:" << std::endl
        ...
        << "                               If omitted, defaults to the latest available version for the device." << std::endl
        << std::endl
        << "Direct flash mode (skip YAML lookup + firmware fetch):" << std::endl
        << "  --bitfile PATH               Path to FPGA bitfile" << std::endl
        ...
        << "  --reapply                    Flash even if the device is already at the target version." << std::endl
        << "                               In direct flash mode, version checking is skipped entirely;" << std::endl
        << "                               --reapply is accepted but has no additional effect." << std::endl
...
bool parse_arguments(int argc, char** argv, hsb_flasher_context& context)
{
    ...
    static struct option long_options[] = {
        ...
        { "bitfile", required_argument, nullptr, OPT_CPNX },
        { "flash-version", required_argument, nullptr, OPT_FLASH_VERSION },
        { "reapply", no_argument, nullptr, OPT_REAPPLY },
        { nullptr, 0, nullptr, 0 }
    };
    ...
    if (context.direct_flash.has_any() && !context.direct_flash.enabled()) {
        log_info("Error: --bitfile is required for direct flash mode (--clnx and --flash-version are optional)");
        return false;
    }
    ...
    if (!context.direct_flash.enabled()) {
        if (!context.target_version.empty()
            && !std::regex_match(context.target_version, std::regex("^[0-9A-Fa-f]+$"))) {
