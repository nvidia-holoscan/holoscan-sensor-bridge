...
CpnxFlash::CpnxFlash(const std::string& context, std::shared_ptr<Hololink> hololink, uint32_t spi_controller_address)
    : MacronixMx25u25645g(context, hololink, spi_controller_address)
{
    ...
    uint32_t read_byte_count)
{
    ...
    auto min_size = command_bytes.size() + read_byte_count;
    if (result.size() < min_size) {
        throw std::runtime_error("SPI transaction returned fewer bytes than expected");
    }
