...
void print_usage(const char* program_name)
{
    std::cout << "Usage: " << program_name << " [OPTIONS] <manifest>" << std::endl
              ...
              << "  --archive=FILE       Use a local zip archive instead of downloading" << std::endl
              << "  --skip-program-clnx  Skip programming CLNX" << std::endl
              << "  --skip-verify-clnx   Skip verifying CLNX" << std::endl
...
int main(int argc, char** argv)
{
    ...
    static struct option long_options[] = {
        ...
        { "archive", required_argument, nullptr, 0 },
        { "skip-program-clnx", no_argument, nullptr, 0 },
        { "skip-verify-clnx", no_argument, nullptr, 0 },
    ...
    while ((c = getopt_long(argc, argv, "h", long_options, &option_index)) != -1) {
        if (c == 0) {
            ...
            } else if (option_name == "archive") {
                args.archive = optarg;
            } else if (option_name == "skip-program-clnx") {
                args.skip_program_clnx = true;
            } else if (option_name == "skip-verify-clnx") {
                args.skip_verify_clnx = true;
