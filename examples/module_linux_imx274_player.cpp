 ...
 *
 * Single-channel IMX274 player driving the hololink_module V1 surface
 * through the SOFTWARE (Linux) receiver. Identical to
 * examples/module_imx274_player.cpp except the receiver operator is
 * hololink::module::operators::LinuxReceiverOp instead of
 * RoceReceiverOp — so this example runs on hosts with no infiniband
 * device (HOLOLINK_BUILD_ROCE=OFF), reassembling HSB's RoCEv2 UDP stream
 * in user space.
 *
 * Discovery is bootp-driven (`Adapter::start_bootp_listener`); the
 * example calls `Adapter::wait_for_channel(peer_ip, timeout)` until the
 * board with `--hololink <ip>` announces itself, then composes a
 * Holoscan pipeline (LinuxReceiverOp → CsiToBayerOp → ImageProcessorOp
