...
public:
    ...
    void compose() override
    {
        ...
        for (size_t idx = 0; idx < sif_metadatas_.size(); ++idx) {
            compose_pipeline(idx, sif_metadatas_[idx], cameras_[idx], window_titles_[idx]);
        }
    }
...
int main(int argc, char** argv)
{
    ...
    std::string sensor_arg = "left"; // 'left' or 'right' — same sensor index from each Hawk
    ...
    // hololink channel(s) setup — one SIF metadata per channel (SIF 0 and SIF 1).
    ...
    auto hawk_a = std::make_shared<Hawk>(legacy_holo, skip_setup);
    auto hawk_b = std::make_shared<Hawk>(legacy_holo, skip_setup);
    ...
    auto application = holoscan::make_application<HoloscanApplication>(headless, fullscreen,
        cu_context, cu_device_ordinal, sif_metadatas, hawk_modules, camera_mode,
