...
public:
    HoloscanStereoApplication(
        ...
        const std::vector<std::shared_ptr<hololink::sensors::NativeImx274Sensor>>& cameras,
    ...
    void compose() override
    {
        ...
        std::vector<std::shared_ptr<hololink::operators::LinuxReceiverOp>> receiver_operators;
        for (size_t i = 0; i < cameras_.size(); ++i) {
            auto receiver_op = make_operator<hololink::operators::LinuxReceiverOp>(
        ...
        for (size_t i = 0; i < cameras_.size(); ++i) {
            ...
            auto image_processor_op = make_operator<hololink::operators::ImageProcessorOp>(
                ...
                Arg("optical_black", 50), // Optical black value for IMX274 is 50 for RAW10
...
int main(int argc, char** argv)
{
    ...
    OptionsDescription options_description("Stereo IMX274 Player Options");

    // clang-format off
    options_description.add_options()
        ("camera-mode", value<int>()->default_value(static_cast<int>(hololink::sensors::imx274_mode::IMX274_MODE_1920X1080_60FPS)), "IMX274 mode (default: 1)")
    ...
    for (size_t i = 0; i < hololink_channels.size(); ++i) {
        int expander_configuration = i;
        cameras.push_back(std::make_shared<hololink::sensors::NativeImx274Sensor>(
            *hololink_channels[i], expander_configuration));
    }

    // Convert camera_mode to proper enum
    auto camera_mode = static_cast<hololink::sensors::imx274_mode::Mode>(variables_map["camera-mode"].as<int>());
