...
public:
    ...
    void compose() override
    {
        ...
        for (size_t idx = 0; idx < per_channel_metadatas_.size(); ++idx) {
            compose_pipeline(idx, per_channel_metadatas_[idx], window_titles_[idx]);
        }
    }
...
int main(int argc, char** argv)
{
    ...
    auto hawk = std::make_shared<Hawk>(sensor_holo, skip_setup);
    ...
    auto application = holoscan::make_application<HoloscanApplication>(headless, fullscreen,
        cu_context, cu_device_ordinal, per_channel_metadatas, hawk, camera_mode,
