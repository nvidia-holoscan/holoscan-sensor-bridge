...
"""Single-camera IMX274 player driving the hololink_module V1 surface
through the SOFTWARE (Linux) receiver.

Identical in shape to the C++ examples/module_linux_imx274_player.cpp:
the pipeline is LinuxReceiverOp -> CsiToBayerOp -> ImageProcessorOp ->
BayerDemosaicOp -> HolovizOp, and the receiver is the software
LinuxReceiverOp (no ibverbs), so this runs on hosts with no infiniband
device.
"""
...
def main():
    ...
    parser.add_argument(
        "--hololink",
        default="192.168.0.2",
        help="Peer IP of the HSB-Lite board (waited for via bootp)",
    )
    parser.add_argument(
        "--expander-configuration",
        type=int,
        default=None,
        help="LI I2C expander output (which camera behind the bus)",
    )
    parser.add_argument(
        "--discovery-timeout",
        type=float,
    ...
    if args.expander_configuration is not None:
        hololink_module.sensors.imx274.Imx274Cam.use_expander_configuration(
            metadata, args.expander_configuration
        )
