 ...
 *
 * Single-channel IMX274 player driving the hololink_module V1
 * surface. Mirrors examples/imx274_player.cpp one-for-one but
 * replaces the legacy `hololink::Enumerator` / `hololink::DataChannel`
 * / `hololink::Hololink` / `hololink::sensors::NativeImx274Sensor`
 * path with the module V1 equivalents:
 *
 *     hololink::module::Adapter (singleton)
 *     hololink::module::HololinkInterfaceV1
 *     hololink::module::sensors::imx274::Imx274Cam
 *     hololink::module::operators::RoceReceiverOp
 *
 * Discovery is bootp-driven (`Adapter::start_bootp_listener`); the
 * example calls `Adapter::wait_for_channel(peer_ip, timeout)` until
 * the board with `--hololink <ip>` announces itself, then composes a
 * Holoscan pipeline (RoceReceiverOp → CsiToBayerOp → ImageProcessorOp
...
int main(int argc, char** argv)
{
    ...
    bool expander_configuration_set = false;
    uint8_t pattern = 0;
    ...
    while (true) {
        ...
        if (c == 0) {
            ...
            if (name == "camera-mode") {
                ...
                module_dir = argument;
            } else if (name == "expander-configuration") {
                expander_configuration = std::stoul(argument);
                expander_configuration_set = true;
            } else if (name == "pattern") {
                pattern = std::stoul(argument);
                pattern_set = true;
            ...
            std::cout << "Usage: " << argv[0] << " [options]\n"
                      ...
                      << "  --expander-configuration <0|1>   LI I2C expander output (which camera behind the bus)\n"
    ...
    // If users requested a specific I2C expander configuration, use that.
    if (expander_configuration_set) {
        hololink::module::sensors::imx274::Imx274Cam::use_expander_configuration(
            metadata, expander_configuration);
    }
