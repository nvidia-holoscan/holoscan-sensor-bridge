...
void mac_from_if(IPAddress& iface, std::string const& if_name)
{
    ...
    memcpy(iface.mac, ether_addr->ether_addr_octet, ETHER_ADDR_LEN);
...
{
    strncpy(dest.if_name, ifa->ifa_name, IFNAMSIZ - 1);
    dest.if_name[IFNAMSIZ - 1] = '\0';
}
...
IPAddress IPAddress_from_string(const std::string& ip)
{
    ...
    mac_from_if(iface, std::string(iface.if_name));
...
uint32_t get_broadcast_address(const IPAddress& ip_address)
{
    ...
    if (!(ip_address.flags & IPADDRESS_HAS_BROADCAST)) {
        if (!((ip_address.flags & IPADDRESS_HAS_NETMASK) && (ip_address.flags & IPADDRESS_HAS_ADDR))) {
            throw std::runtime_error("broadcast address cannot be determined: missing addr or netmask for interface " + std::string(ip_address.if_name));
        }
