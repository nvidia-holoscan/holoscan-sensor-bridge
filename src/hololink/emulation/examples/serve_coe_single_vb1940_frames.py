...
from emulator_utils import LoopConfig, loop_single_vb1940
...
from single vb1940 sensor (e.g. Leopard Eagle HSB)
...
def main():
    parser = argparse.ArgumentParser(
        description="Serve CSI-2 data as if from a single vb1940 sensor"
    )
    parser.add_argument(
        "ip_address",
        type=str,
    ...
    coe_data_plane = hemu.COEDataPlane(
        hsb, hemu.IPAddress(args.ip_address), data_plane_id, sensor_id
    )

    # create and attach the vb1940 sensor to the HSB's I2C controller
    ...
    try:
        loop_single_vb1940(loop_config, vb1940, coe_data_plane, args.gpu)
