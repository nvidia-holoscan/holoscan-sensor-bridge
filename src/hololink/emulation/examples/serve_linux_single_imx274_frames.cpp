...
void print_usage(char const* program_name)
{
    printf("Usage: %s [options] <hololink_ip_address>\n"
           "    serves csi-2 data as if from a single imx274 sensor from ip address 'hololink_ip_address'\n\n"
...
int main(int argc, char** argv)
{
    ...
    HSBEmulator hsb(HSB_EMULATOR_CONFIG);
    ...
    sensors::IMX274Emulator imx274;
    // The i2c bus address is the sensor_id offset from CAM_I2C_BUS
    imx274.attach_to_i2c(hsb.get_i2c(hololink::I2C_CTRL), hololink::CAM_I2C_BUS + sensor_id);
    ...
    loop_single_imx274(loop_config, imx274, linux_data_plane, tensor);
