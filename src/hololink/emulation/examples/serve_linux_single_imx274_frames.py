...
from emulator_utils import LoopConfig, loop_single_imx274
...
def main():
    parser = argparse.ArgumentParser(
        description="Serve CSI-2 data as if from a single imx274 sensor using Linux data plane"
    )
    parser.add_argument(
        "ip_address",
        type=str,
    ...
    hsb = hemu.HSBEmulator(hemu.HSB_EMULATOR_CONFIG)
    data_plane_id = 0
    sensor_id = 0

    linux_data_plane = hemu.LinuxDataPlane(
        hsb, hemu.IPAddress(args.ip_address), data_plane_id, sensor_id
    )

    # create and attach the imx274 sensor to the HSB's I2C controller
    imx274 = hemu.sensors.IMX274Emulator()
    # The i2c bus address is the sensor_id offset from CAM_I2C_BUS
    imx274.attach_to_i2c(
    ...
    try:
        loop_single_imx274(loop_config, imx274, linux_data_plane, args.gpu)
