...
/*
@brief Cross-platform HSB Emulator that emits a message buffer over the RoCEv2 data plane every loop iteration.

Each transmitted buffer is the concatenation of a 4-byte network byte order uint32_t loop counter followed by
...
{
    HSBEmulator hsb;
    RoCEv2DataPlane data_plane(hsb, IPAddress_from_string(IPV4_ADDRESS), DATA_PLANE_ID, SENSOR_ID);
