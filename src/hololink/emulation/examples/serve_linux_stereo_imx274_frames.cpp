...
void print_usage(char const* program_name)
{
    printf("Usage: %s [options] <hololink_ip_address>\n"
           "    serves csi-2 data as if from a stereo imx274 sensor from ip address 'hololink_ip_address'\n\n"
...
int main(int argc, char** argv)
{
    ...
    struct HSBConfiguration config = HSB_EMULATOR_CONFIG;
    ...
    const uint8_t sensor_id_0 = 0;
    const uint8_t sensor_id_1 = 1;
    auto address = IPAddress_from_string(ip_address);
    LinuxDataPlane linux_data_plane_0(
        hsb,
        address,
        data_plane_id,
        sensor_id_0);
    LinuxDataPlane linux_data_plane_1(
        hsb,
        address,
        data_plane_id,
        sensor_id_1);

    sensors::IMX274Emulator imx274_0;
    sensors::IMX274Emulator imx274_1;
    // The i2c bus address is the sensor_id offset from CAM_I2C_BUS
    imx274_0.attach_to_i2c(hsb.get_i2c(hololink::I2C_CTRL), hololink::CAM_I2C_BUS + sensor_id_0);
    imx274_1.attach_to_i2c(hsb.get_i2c(hololink::I2C_CTRL), hololink::CAM_I2C_BUS + sensor_id_1);
    ...
    loop_stereo_imx274(loop_config, imx274_0, linux_data_plane_0, tensor_0, imx274_1, linux_data_plane_1, tensor_1);
