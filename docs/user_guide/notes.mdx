...
- PTP timestamps, sent by HSB on both the start-of-frame and the end-of-frame condition,
  ...
  displaying this data. Note that PTP is only supported for IGX configurations.
...
[demo container](/holoscan/sensor-bridge/getting-started/build#RunningTestsTarget):
...
First, follow the [setup instructions](setup.mdx) to configure the host IP addresses and
...
[Host setup](setup.mdx) page.
[Here is more specific information on sensor bridge IP address reconfiguration](/holoscan/sensor-bridge/applications/architecture#datachannel-enumeration-and-ip-address-configuration).

## Network cables and adapters

Please use one of the following cables to connect the sensor bridge SFP+ to the **IGX
...
1. [Fiber optic cable](https://www.amazon.com/FLYPROFiber-10ft-4pack-Fiber-Length-Options/dp/B089K1J5GG)
   with
   [optic to SFP+](https://www.amazon.com/Multi-Mode-Transceiver-10GBASE-SR-SFP-10G-SR-Supermicro/dp/B01N1H1Z2F?th=1)
   and
...
Configuring the receiver to accept larger MTUs usually requires OS-level configuration
of MTU on the receiving network interface.
[Host setup instructions](/holoscan/sensor-bridge/getting-started/host-setup#HostSetupTarget)
now include recommendations to execute e.g.
