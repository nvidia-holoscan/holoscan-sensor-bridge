...
When enumeration data is observed from the given IP address, information about the found
device is returned into the `channel_metadata` variable.
...
Note that in this case the IMX274 sensor is implemented using Python, the code below
shows how to create the Python object in C++.
...
- In our sample application, we need to initialize the camera clock and configure the
  image format the camera will transmit. Note that in the IMX274 stereo camera, the same
  clock is used to drive both camera devices, so care must be taken to ensure you don't
...
[Specific information about host network configuration can be found here.](notes.mdx#holoscan-sensor-bridge-ip-address-configuration)
...
[description of HSB latency](latency.mdx) for details on the contents and uses of this
data.
