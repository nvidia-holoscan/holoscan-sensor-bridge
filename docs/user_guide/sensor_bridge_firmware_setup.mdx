...
1. Follow the [setup instructions](setup.mdx) to build and run the demo container. All
...
1. Check connectivity with the sensor bridge board with the ping command:
   ...
   for [configuring the IP addresses used by the sensor bridge device.](notes.mdx) After
...
1. These instructions assume that your FPGA was loaded with 0x2412 or newer firmware;
   this is the version included with HSB 2.0. If your configuration is older, follow the
...
In normal usage as shown in
[Quick Start](/holoscan/sensor-bridge/firmware/firmware-setup#quick-start),
`hsb_flasher` performs the following steps:
