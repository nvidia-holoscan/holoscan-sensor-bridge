#----------------------------------------------------------------------------------------------
# FPGA Version v01.1.0
#----------------------------------------------------------------------------------------------

set prj_name    fpga_mipi_cpnx_ref_design
set vendor_ID   0000
...
exec ipgen -o ../../ip/mipi_rx_ip \
           -vlnv latticesemi.com:ip:dphy_rx:2.1.0 \
           -name mipi_rx_ip \
           -cfg "../../ip/mipi_rx_ip/mipi_rx_ip.cfg" \
           -a LFCPNX -p LFCPNX-100 -t CBG256 -sp 9_High-Performance_1.0V -op COM

exec ipgen -o ../../ip/mipiClkOutPLL \
        -ip $env(RADIANT_PATH)/ip/lifcl/pll \
        -name mipiClkOutPLL \
        -cfg "../../ip/mipiClkOutPLL/mipiClkOutPLL.cfg" \
        -a LFCPNX -p LFCPNX-100 -t CBG256 -sp 9_High-Performance_1.0V -op COM

exec ipgen -o ../../ip/eclk_mipi_pll \
           -ip $env(RADIANT_PATH)/ip/lifcl/pll \
           -name eclk_mipi_pll \
           -cfg "../../ip/eclk_mipi_pll/eclk_mipi_pll.cfg" \
           -a LFCPNX -p LFCPNX-100 -t CBG256 -sp 9_High-Performance_1.0V -op COM
...
set src_pdc "build/constraints/fpga_cpnx.pdc"
addFiles $src_dir $src_pdc

set src_sdc "build/constraints/fpga_cpnx.sdc"
...
file copy ./impl_1/mipi_cpnx_ref_design_impl_1.bit ../../../bitfile/mipi_cpnx_ref_design_$dateTime.bit
